...
def prepare_cbp_kwargs(net, net_args):
    return {
        'linear_layers': [list(net.dueling.value_branch[::2].children()), list(net.dueling.advantage_branch[::2].children())],
        'activation_layers': [list(net.dueling.value_branch[1::2].children()), list(net.dueling.advantage_branch[1::2].children())],
...
def _generate_step_function(sample_weights):
    ...
    '''
    @torch.jit.script
    def step_calcs(cbp_vals: Dict[str, torch.Tensor],
...
#        self.is_training = is_training
    ...
    def step(self):
        ...
        for linears in self.linear_layers: # cycle through models
            for current_linear, next_linear in zip(linears[:-1], linears[1:]): # cycle through layers
                cbp_vals = self.cbp_vals[current_linear]
                pre_state = self.state[current_linear.weight]
                post_state = self.state[next_linear.weight]
                
                self.step_calcs(cbp_vals, pre_state, post_state, current_linear.weight, next_linear.weight, self.eta, self.m, self.rho, self.eps) #self.sample_weights)
        
    def _hook_gen(self, linear_layer):
        num_units = linear_layer.weight.shape[0]
        self.cbp_vals[linear_layer] = {
            'age':  torch.ones(num_units, dtype=int, device=self.dev, requires_grad=False), 
    ...
    def _add_hooks(self, linears, activations):
        assert len(linears) == len(activations) + 1 #1 to 1 linear and activation, plus output linear
