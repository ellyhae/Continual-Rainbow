...
from stable_baselines3.common.utils import get_linear_fn, get_parameters_by_name
...
class Rainbow(AsyncOffPolicyAlgorithm):
    ...
    def __init__(
        ...
        learning_rate: Union[float, Schedule] = 0.00025,           # lr, lr_decay_steps, lr_decay_factor
        ...
        replay_buffer_class: Optional[Type[BaseBuffer]] = PrioritizedReplayBuffer, # prioritized_er
        ...
        prioritized_er_beta0_final: float = 1.,                    # set prioritized_er_beta0_initial == prioritized_er_beta0_final for constant value
        prioritized_er_beta0_fraction: float = 1.,
        ...
        policy_kwargs: Optional[Dict[str, Any]] = {'noisy_linear': True, 'linear_kwargs': {'sigma_0': 0.5}, 'optimizer_kwargs': {'eps': None}}, # noisy_dqn, noisy_sigma0, adam_eps
        verbose: int = 0,
        ...
        assert self.target_update_interval % self.n_envs == 0, "target would not be synced since the main loop iterates in steps of parallel_envs"
        self._game_frame = 0
    ...
    def _on_step(self) -> None:
        ...
        if (self.num_timesteps-self.n_envs) % self.target_update_interval == 0 and self.replay_buffer.size() > self.learning_starts:
    ...
    def _td_target(self, reward: float, next_state, done: bool):
        ...
        if self.double_dqn:
            best_action = torch.argmax(self.q_net(next_state, advantages_only=True), dim=1)   # could maybe replace with self.policy._predict
            next_Q = torch.gather(self.q_net_target(next_state), dim=1, index=best_action.unsqueeze(1)).squeeze()
    ...
    def train(self, gradient_steps: int, batch_size: int = 100) -> None:
        ...
        for train_iter in range(gradient_steps):
            ...
            if self.prioritized_er:
                indices, weights, replay_data = self.replay_buffer.sample(batch_size, self.per_beta, env=self._vec_normalize_env)
            ...
            with autocast(enabled=self.use_amp):
                ...
                td_est = torch.gather(self.q_net(replay_data.observations), dim=1, index=replay_data.actions).squeeze()
                td_tgt = self._td_target(replay_data.rewards.squeeze(1), replay_data.next_observations, replay_data.dones.squeeze(1))
    ...
    def learn(
        ...
        return super().learn(
            ...
            callback=NoiseReset() if callback is None else [NoiseReset()] + (callback if type(callback) == list else [callback]),
    ...
    def _excluded_save_params(self) -> List[str]:
        return [*super()._excluded_save_params(), "q_net", "q_net_target", "policy.optimizer.step_calcs", "policy.optimizer.sample_weights"]
