...
def create_mlp(
    ...
    if len(net_arch) > 0:
        modules = [linear_layer(input_dim, net_arch[0], **linear_kwargs), activation_fn()]
    else:
        modules = []
...
class ImpalaCNNLarge(BaseFeaturesExtractor):
    ...
    def __init__(
        ...
        self.main = torch.nn.Sequential(
            ...
            activation_fn(),
            torch.nn.AdaptiveMaxPool2d((8, 8)), # reduces output to shape (batch_size, 32*model_size, 8, 8)
            torch.nn.Flatten()
        )
        ...
        if features_dim is not None:
            self.last = nn.Sequential(torch.nn.Linear(n_flatten, features_dim), torch.nn.ReLU())
