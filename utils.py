...
class CBPLogger(BaseCallback):
    def __init__(self, save_dir, logging_freq=500, verbose=0):
        '''
        ...
        '''
        super(CBPLogger, self).__init__(verbose)
    ...
    def _init_callback(self):
        ...
        if self.active:
            self.ages = {}
            for branch, branch_name in zip(self.model.policy.optimizer.linear_layers, ['value', 'advantage']):
                for i, layer in enumerate(branch[:-1]):
                    self.ages[f'{branch_name}_{i}'] = self.model.policy.optimizer.cbp_vals[layer]['age'].detach().clone()
    ...
    def _save(self):
        if self.active and self.model.replay_buffer.size() > self.model.learning_starts:
            ...
            for branch, branch_name in zip(opt.linear_layers, ['value', 'advantage']):
                for i, layer in enumerate(branch[:-1]):
                    self.ages[f'{branch_name}_{i}'].add_(self.model.gradient_steps) # simulate updates without resets
                    cbp_vals = opt.cbp_vals[layer]
                    val = cbp_vals['age']
                    if (self.ages[f'{branch_name}_{i}'] != val).any() or self.end:
                        self.ages[f'{branch_name}_{i}'].copy_(val)
                        self.logger.record(f'age/{branch_name}_{i}', val.cpu().numpy(), exclude='tensorboard')
                        age_logged = True
                    if self.iteration % self.logging_freq == 0 or self.end:
                        for name, val in cbp_vals.items():
                            self.logger.record(f'{name}/{branch_name}_{i}', val.cpu().numpy(), exclude='tensorboard')
            ...
            if logged or age_logged:
                self.logger.dump(self.num_timesteps)
            if logged:
                torch.save(self.ages, os.path.join(self.save_dir, str(self.iteration) + '_vals.pt'))
            if age_logged:
                torch.save(self.ages, os.path.join(self.save_dir, str(self.iteration) + '_age.pt'))
                ...
                if len(opt.state) > 0:
                    ...
                    for branch, branch_name in zip(opt.linear_layers, ['value', 'advantage']):
                        for i, layer in enumerate(branch):
                            steps[f'{branch_name}_{i}'] = opt.state[layer.weight]['step']
                    torch.save(steps, os.path.join(self.save_dir, str(self.iteration) + '_step.pt'))
